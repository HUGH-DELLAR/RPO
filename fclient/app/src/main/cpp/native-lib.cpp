...
extern "C" JNIEXPORT jstring JNICALL // этот метод преобразует строку из ASCIIZ в UTF и возвращает её в код на Java
Java_ru_iu3_fclient_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
    std::string hello = "Hello from C++";
    LOG_INFO("Hello from system log %d", 2021); //происходит вывод в стандартный лог
    SLOG_INFO("Hello from spdlog {}", 2021); //происходит вывод в spdlog
    return env->NewStringUTF(hello.c_str());
}

extern "C" JNIEXPORT jint JNICALL
Java_ru_iu3_fclient_MainActivity_initRng( //инициализация генератора
        JNIEnv* env, jclass) {

    mbedtls_entropy_init(&entropy);
    mbedtls_ctr_drbg_init(&ctr_drbg);

    return mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func,//задаёт случайную позицию, с которой
            //начнется генерация псевдослучайной последовательности
            &entropy, (const unsigned char*) personalization,
            strlen(personalization));
}

extern "C" JNIEXPORT jbyteArray JNICALL
Java_ru_iu3_fclient_MainActivity_randomBytes(JNIEnv *env, jclass /* this */, jint no) {//возвращает случайные байты
...
{
// здесь реализован алгоритм шифрования 3DES
